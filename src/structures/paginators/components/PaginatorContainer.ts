...
export interface PaginatorContainerConfiguration {
...
const FIRST_PAGE_ID = "__paginator_container_first__";
const PREV_PAGE_ID = "__paginator_container_prev__";
const NEXT_PAGE_ID = "__paginator_container_next__";
const LAST_PAGE_ID = "__paginator_container_last__";
...
export class PaginatorContainer {
	...
	constructor({
		...
		if (pages.length === 0) {
			throw new Error("PaginatorContainer: at least one page is required.");
		}
	...
	private attachCollector(message: Message, interaction?: RepliableInteraction, filterUserId?: string): void {
		...
		collector.on("end", async () => {
			const disabledPayload = {
				components: [this.buildCurrentContainer(true)],
	...
	private buildCurrentContainer(disabled = false): ContainerBuilder {
		...
		return container
			...
			.addActionRowComponents(...this.buildPaginatorComponents(disabled));
	}

	private buildPaginatorComponents(disabled = false): ActionRowBuilder<MessageActionRowComponentBuilder>[] {
